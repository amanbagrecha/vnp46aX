...
def get_tiles(bounds):
    ...
    gdf = gpd.read_file("BlackMarbleTiles/BlackMarbleTiles.shp")
    print(f"{bounds=}")
    bbox = box(*bounds)
    gdf_up = gdf[gdf.intersects(bbox)==True]
    return gdf_up.TileID.values
...
def sync(src, dest, tok, bounds):
    ...
    for f in files:
        tile_list = get_tiles(bounds)
        if not any(ext in f['name'] for ext in tile_list):
            print('skipping: ', f)
            continue
...
def _main(argv):
    ...
    parser.add_argument('-b', '--bounds', dest='bounds', metavar='BOX',nargs=4, type=int, help='Specify bounds as "xmin,ymin,xmax,ymax" ', required=True)
    args = parser.parse_args(argv[1:])
    if not os.path.exists(args.destination):
        os.makedirs(args.destination)
    return sync(args.source, args.destination, args.token, args.bounds)
